...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🚀 Enviando invitación a ${email} desde registros@rivapp.com.ar`)

    // 3. Enviar email usando RESEND (Usando tu dominio verificado)
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Rivapp <registros@rivapp.com.ar>', // 🟢 TU DOMINIO REAL
        to: [email],
        ...
        html: `
          <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
            ...
            <div style="background-color: #f9f9f9; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
                ...
                <p style="font-size: 18px; font-weight: bold; margin: 0;">${role.toUpperCase()}</p>
                ${branch_name ? `<p style="margin-top: 10px; color: #666;">📍 ${branch_name}</p>` : ''}
