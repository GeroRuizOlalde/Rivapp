    ...
    {[1, 2, 3, 4, 5].map((star) => (
      ...
      >
        <Star
          ...
          color={star <= rating ? color : '#4b5563'}
          strokeWidth={1.5}
        />
      </button>
    ))}
...
export default function Tracking() {
  ...
  const confettiPieces = useMemo(
    () =>
      Array.from({ length: 20 }, (_, index) => ({
        ...
        delay: (index % 6) * 0.15,
        color: [brandColor, '#3b82f6', '#ef4444', '#10b981'][index % 4],
      })),
    [brandColor]
  );
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center text-white p-6 text-center">
        ...
        <p className="text-gray-500 mb-6 text-sm">El enlace podria ser antiguo o incorrecto.</p>
        <Link
          to="/"
          className="text-black px-8 py-3 rounded-full font-bold shadow-lg"
  ...
  const steps = [
    ...
    { key: 'entregado', label: 'Llego', icon: <Check size={16} /> },
  ];
        ...
        <div className="p-5 flex-1 flex flex-col">
          ...
          <div className="bg-[#1a1a1a] rounded-3xl p-6 border border-white/5 shadow-lg mb-6 relative overflow-hidden">
            {order.status === 'rechazado' ? (
              <div className="text-red-500 font-bold flex flex-col items-center gap-3 py-4">
                ...
                <p className="text-sm text-gray-400 font-normal px-4 text-center">
                  ...
                  {steps.map((step, index) => {
                    ...
                    return (
                      <div key={step.key} className="flex flex-col items-center gap-2">
                        ...
                        <span className={`text-[10px] font-bold uppercase transition-colors ${isActive ? 'text-white' : 'text-gray-600'}`}>
                          {step.label}
                        </span>
                      </div>
                    );
          ...
          {order.rider_id && order.riders && (
            <motion.div
              ...
              className="bg-gradient-to-r from-blue-900/20 to-blue-800/10 rounded-3xl p-5 border border-blue-500/20 mb-6 flex items-center justify-between"
            >
              ...
              {order.riders.phone && (
                <a
                  ...
                  className="bg-white text-blue-900 p-3 rounded-xl font-bold hover:bg-blue-50 transition-colors shadow-md"
                >
                  <MessageSquare size={20} />
                </a>
              )}
          ...
          <div className="bg-[#1a1a1a] rounded-3xl p-6 border border-white/5 shadow-lg mb-6">
              ...
              {order.items?.map((item, index) => (
                <div key={index} className="flex justify-between items-start">
                  <div className="flex gap-3">
                    <div className="font-bold text-black w-6 h-6 flex items-center justify-center rounded-md text-xs" style={{ backgroundColor: brandColor }}>
                      {item.quantity}
                    </div>
                    <div>
            ...
            <div className="pt-4 border-t border-white/10 space-y-2">
              ...
              <div className="flex justify-end gap-2 mt-1">
                <span className="text-[10px] bg-white/5 text-gray-400 px-2 py-1 rounded uppercase font-bold">
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mt-2 mb-8">
              {!order.rating ? (
                <button
                  ...
                  className="w-full py-4 rounded-2xl font-bold text-black shadow-xl transition-transform active:scale-95 flex items-center justify-center gap-2"
                  style={{ backgroundColor: brandColor }}
                >
                ...
                <div className="bg-green-500/10 border border-green-500/20 p-4 rounded-2xl text-center">
                  <p className="text-green-500 font-bold flex items-center justify-center gap-2">
                    <Check size={18} /> Gracias por tu opinion
                  </p>
                </div>
              )}
            </motion.div>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm">
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-gray-400 mb-8">Tu pedido ya esta confirmado y en camino a la cocina.</p>
          ...
          {showRatingModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="bg-[#1a1a1a] w-full max-w-sm rounded-[2rem] border border-white/10 p-6 relative z-10 shadow-2xl"
              >
                <button
                  onClick={() => setShowRatingModal(false)}
                  className="absolute top-4 right-4 bg-white/5 p-2 rounded-full text-gray-400 hover:text-white"
                >
                  <X size={20} />
                </button>
                <div className="text-center mb-6 mt-2">
                  <h3 className="text-xl font-bold text-white mb-1">Que tal estuvo?</h3>
                  <p className="text-xs text-gray-400">Ayudanos a mejorar</p>
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 rounded-xl font-bold text-black shadow-lg transition-all active:scale-95 disabled:opacity-50"
                  style={{ backgroundColor: brandColor }}
                >
                  {isSubmitting ? <Loader2 className="animate-spin mx-auto" /> : 'Enviar Resena'}
                </button>
              </motion.div>
            </div>
          )}
