...
import { Lock, Store, ArrowRight, Loader2, LogOut, Menu as MenuIcon } from 'lucide-react';
...
export default function AdminGastronomy() {
  ...
  const toggleSound = useCallback(() => {
    ...
    if (newState) {
      audioRef.current.play().then(() => { audioRef.current.pause(); audioRef.current.currentTime = 0; }).catch((e) => logger.debug('Audio permission pending', e));
    }
  }, [isSoundEnabled]);
  ...
  const playNotification = useCallback(() => {
    ...
    if (soundEnabledRef.current) { audioRef.current.currentTime = 0; audioRef.current.play().catch(e => logger.error("Error audio:", e)); }
  }, []);

  // --- Fetch Functions ---
  const fetchOrders = useCallback(async () => {
    if (!config) return;
    const { data } = await supabase.from('orders').select('*, riders(name)').eq('store_id', config.id).neq('status', 'archivado').order('created_at', { ascending: false });
    if (data) {
      setOrders(data);
  ...
  const fetchRiders = useCallback(async () => { const { data } = await supabase.from('riders').select('*, branches(name)').eq('store_id', config.id).order('id'); if (data) setRiders(data || []); }, [config?.id]);
  ...
  const fetchHistory = useCallback(async (reset = true) => {
    ...
    const { data } = await supabase.from('orders').select('*').eq('store_id', config.id)
  ...
  const fetchReviews = useCallback(async () => { const { data } = await supabase.from('orders').select('*').eq('store_id', config.id).not('rating', 'is', null).order('created_at', { ascending: false }).limit(100); if (data) setReviews(data || []); }, [config?.id]);
  const fetchCRMData = useCallback(async () => { if (!config?.id) return; const { data } = await supabase.from('customer_insights').select('*').eq('store_id', config.id).order('total_orders', { ascending: false }).limit(200); if (data) setCustomers(data); }, [config?.id]);
  const fetchTeamInvites = useCallback(async () => { const { data } = await supabase.from('team_invitations').select('*').eq('store_id', config.id).order('created_at', { ascending: false }); if (data) setTeamInvites(data); }, [config?.id]);
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      try { const parsed = JSON.parse(localSession); if (parsed) { setSession(parsed); setLoadingSession(false); return; } } catch { /* ignore */ }
      const { data: { session: authSession } } = await supabase.auth.getSession();
      if (authSession?.user) { const sessionData = { id: authSession.user.id, email: authSession.user.email }; setSession(sessionData); localStorage.setItem('rivapp_session', JSON.stringify(sessionData)); }
  ...
  useEffect(() => {
    ...
    fetchMenu(); fetchHistory(true); fetchCoupons(); fetchRiders(); fetchReviews(); fetchGlobalNotifications(); fetchCRMData(); fetchBranches(); fetchTeamInvites();
    setSettingsForm({
      store_name: config.name, color_accent: config.color_accent, cbu_alias: config.cbu_alias, phone: config.phone || '',
      logo_url: config.logo_url, banner_url: config.banner_url, lat: config.lat, lng: config.lng,
      schedule_start: config.schedule_start, schedule_end: config.schedule_end, auto_schedule: config.auto_schedule,
      mp_access_token: '', mp_public_key: '', mp_client_id: '', mp_client_secret: '',
      delivery_base_price: config.delivery_base_price || 500, delivery_price_per_km: config.delivery_price_per_km || 300, charge_delivery_in_mp: config.charge_delivery_in_mp ?? true
    });
  ...
  const filteredOrders = useMemo(() => !viewBranchId ? orders : orders.filter(o => o.branch_id === viewBranchId), [orders, viewBranchId]);
  const filteredHistory = useMemo(() => !viewBranchId ? historyOrders : historyOrders.filter(o => o.branch_id === viewBranchId), [historyOrders, viewBranchId]);
  ...
  const calculateDashboardStats = useCallback(() => {
    ...
    let filtered = [];
    const now = new Date();
    if (statsFilter === 'today') { const todayStr = now.toLocaleDateString('en-CA'); filtered = validOrders.filter(o => new Date(o.created_at).toLocaleDateString('en-CA') === todayStr); }
    else if (statsFilter === 'week') { const d = new Date(); d.setDate(d.getDate() - 7); filtered = validOrders.filter(o => new Date(o.created_at) >= d); }
    else if (statsFilter === 'month') { const m = now.toISOString().slice(0, 7); filtered = validOrders.filter(o => o.created_at?.startsWith(m)); }
    const totalOrders = filtered.length;
    const totalRevenue = filtered.reduce((sum, o) => sum + (o.total || 0), 0);
    const avgTicket = totalOrders > 0 ? totalRevenue / totalOrders : 0;
    const chartMap = {};
    filtered.forEach(o => { const dateObj = new Date(o.created_at); const key = statsFilter === 'today' ? `${dateObj.getHours()}:00` : `${dateObj.getDate()}/${dateObj.getMonth() + 1}`; chartMap[key] = (chartMap[key] || 0) + (o.total || 0); });
  ...
  const getBranchLocation = () => { if (navigator.geolocation) { navigator.geolocation.getCurrentPosition((pos) => { setBranchForm(prev => ({ ...prev, lat: pos.coords.latitude, lng: pos.coords.longitude })); alert("Ubicacion detectada"); }); } };
  ...
  const handleUpdateProduct = async (e) => { e.preventDefault(); if (!editingProduct) return; const { error } = await supabase.from('menu').update(editingProduct).eq('id', editingProduct.id).eq('store_id', config.id); if (!error) { alert("Actualizado!"); setShowEditProductModal(false); setEditingProduct(null); fetchMenu(); } };
  const deleteProduct = async (id) => { if (window.confirm("¿Borrar?")) { await supabase.from('menu').delete().eq('id', id).eq('store_id', config.id); fetchMenu(); } };
  const updateProductField = async (id, f, v) => { setMenuItems(p => p.map(i => i.id === id ? { ...i, [f]: v } : i)); await supabase.from('menu').update({ [f]: v }).eq('id', id).eq('store_id', config.id); };
  ...
  const updateOrderStatus = async (id, status) => { await supabase.from('orders').update({ status }).eq('id', id).eq('store_id', config.id); fetchOrders(); };
  const handleMarkAsPaid = async (id) => { if (!window.confirm("¿Confirmar que el cliente realizó el pago?")) return; await supabase.from('orders').update({ paid: true, payment_status: 'paid' }).eq('id', id).eq('store_id', config.id); fetchOrders(); };
  const handleRejectOrder = async (id) => { const reason = window.prompt("Motivo:"); if (!reason) return; await supabase.from('orders').update({ status: 'rechazado', rejection_reason: reason }).eq('id', id).eq('store_id', config.id); fetchOrders(); };
  const handlePrintOrder = (order) => { const popupWin = window.open('', '_blank', 'width=350,height=600'); const itemsHtml = (order.items || []).map((item) => `<tr><td>${item.quantity}</td><td>${item.name}</td><td>$${(item.finalPrice * item.quantity).toLocaleString()}</td></tr>`).join(''); const html = `<html><body style="font-family: monospace"><h2>${config.name}</h2><p>Pedido #${order.id.slice(0, 6)}</p><hr /><table style="width: 100%">${itemsHtml}</table><hr /><h3>TOTAL: $${order.total}</h3><p>${order.note ? `NOTA: ${order.note}` : ''}</p><script>window.print();setTimeout(window.close, 500);</script></body></html>`; popupWin.document.write(html); popupWin.document.close(); };
  ...
  const saveSettings = async (e) => {
    ...
    try {
      ...
      if (mp_access_token || mp_public_key || mp_client_id || mp_client_secret) {
        const { error: fnError } = await supabase.functions.invoke('save-mp-settings', { body: { store_id: config.id, mp_access_token, mp_public_key, mp_client_id, mp_client_secret } });
    ...
    } catch (error) { logger.error(error); alert("Error al guardar: " + error.message); }
  };
  ...
  const deleteRider = async (id) => { if (window.confirm("¿Borrar?")) { await supabase.from('riders').delete().eq('id', id).eq('store_id', config.id); fetchRiders(); } };
  const handleAssignRider = async (riderId) => { if (!selectedOrderForRider) return; await supabase.from('orders').update({ rider_id: riderId, status: 'listo' }).eq('id', selectedOrderForRider.id).eq('store_id', config.id); setShowAssignRiderModal(false); fetchOrders(); };
  ...
  const deleteCoupon = async (id) => { if (window.confirm("¿Borrar?")) { await supabase.from('coupons').update({ active: false }).eq('id', id).eq('store_id', config.id); fetchCoupons(); } };
  ...
  const saveOrderChanges = async (e) => { e.preventDefault(); await supabase.from('orders').update({ customer_name: editingOrder.customer_name, total: parseFloat(editingOrder.total), payment_method: editingOrder.payment_method, status: editingOrder.status, note: editingOrder.note }).eq('id', editingOrder.id).eq('store_id', config.id); setShowEditOrderModal(false); fetchHistory(true); fetchOrders(); };
  const handleDeleteSingleOrder = async (id) => { if (window.confirm("¿Borrar?")) { await supabase.from('orders').delete().eq('id', id).eq('store_id', config.id); fetchHistory(true); } };
  const handleDeleteAllHistory = async () => { if (window.confirm("¿Vaciar todo?")) { await supabase.from('orders').delete().eq('store_id', config.id).or('status.eq.archivado,status.eq.rechazado,status.eq.entregado'); fetchHistory(true); } };

  const handleInviteMember = async (e) => {
    e.preventDefault();
    if (!newMember.email) return alert("Falta el email");
    const btn = document.activeElement; const originalText = btn.innerText; btn.innerText = "Enviando..."; btn.disabled = true;
    try {
      const payload = { store_id: config.id, email: newMember.email, role: newMember.role, branch_id: (newMember.role === 'manager' || newMember.role === 'staff') ? newMember.branch_id : null, status: 'pending' };
      ...
      const { error: fnError } = await supabase.functions.invoke('invite-user', { body: { email: newMember.email, role: newMember.role, store_name: config.name, branch_name: branchName, invite_link: inviteLink } });
      if (fnError) { logger.error("Error enviando mail:", fnError); alert("Invitación guardada, pero falló el envío del correo."); }
      else { alert(`Invitación enviada a ${newMember.email}!`); }
      setShowTeamModal(false); setNewMember({ email: '', role: 'staff', branch_id: '' }); fetchTeamInvites();
    } catch (err) { alert("Error: " + err.message); }
    finally { btn.innerText = originalText; btn.disabled = false; }
  };
  const handleDeleteInvite = async (id) => { if (!window.confirm("¿Eliminar invitación?")) return; await supabase.from('team_invitations').delete().eq('id', id); fetchTeamInvites(); };
  ...
  return (
    <div className={`min-h-screen bg-[#0f0f0f] text-white flex flex-col lg:flex-row relative ${flash ? 'bg-orange-900/50' : ''} transition-colors duration-200`}>
      ...
      <NotificationToast toasts={toasts} onDismiss={dismissToast} onClickToast={(toast) => { const tab = NOTIFICATION_TAB_MAP[toast.type]; if (tab) setActiveTab(tab); markAsRead(toast.id); }} />
      ...
      <main className="flex-1 p-4 lg:p-8 overflow-y-auto h-screen bg-[#050505] pb-24 lg:pb-0">
        <div className="lg:hidden flex justify-between items-center mb-6">
          <div className="w-2/3"><AdminBranchSelector selectedBranchId={viewBranchId} onSelect={handleBranchChange} /></div>
        </div>

        {activeTab === 'dashboard' && <DashboardTab config={config} role={role} accentColor={accentColor} viewBranchId={viewBranchId} getBranchName={getBranchName} dashboardData={dashboardData} globalNotifications={globalNotifications} onDismissMessage={dismissMessage} storeNotifications={storeNotifications} unreadCount={unreadCount} onMarkAsRead={markAsRead} onMarkAllAsRead={markAllAsRead} onDeleteNotification={deleteNotification} onClearAllNotifications={clearAllNotifications} onCloseRegister={handleCloseRegister} />}
        {activeTab === 'orders' && <OrdersTab filteredOrders={filteredOrders} isSoundEnabled={isSoundEnabled} onToggleSound={toggleSound} onRefreshOrders={fetchOrders} onUpdateOrderStatus={updateOrderStatus} onRejectOrder={handleRejectOrder} onPrintOrder={handlePrintOrder} onMarkOrderPaid={handleMarkAsPaid} onOpenAssignRider={(order) => { setSelectedOrderForRider(order); setShowAssignRiderModal(true); }} getBranchName={getBranchName} />}
        {activeTab === 'team' && <TeamTab teamInvites={teamInvites} onOpenRolesModal={() => setShowRolesModal(true)} onOpenInviteModal={() => setShowTeamModal(true)} getBranchName={getBranchName} onDeleteInvite={handleDeleteInvite} />}
        {activeTab === 'menu' && <MenuTab selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} menuItems={menuItems} onOpenPriceModal={() => setShowPriceModal(true)} onOpenCreateProductModal={() => setShowCreateModal(true)} onOpenPromoModal={(item) => { setPromoTargetItem(item); setShowPromoModal(true); }} onToggleAvailability={updateProductField} onOpenEditProductModal={(item) => { setEditingProduct(item); setShowEditProductModal(true); }} onDeleteProduct={deleteProduct} />}
        {activeTab === 'crm' && <CRMTab customers={customers} onExportCustomers={exportCustomers} />}
        {activeTab === 'riders' && <RidersTab riders={riders} branches={branches} accentColor={accentColor} contrastTextColor={contrastTextColor} onCreateRider={() => { setNewRider({ name: '', phone: '', access_pin: '', branch_id: '' }); setShowCreateRiderModal(true); }} onDeleteRider={deleteRider} />}
        {activeTab === 'coupons' && <CouponsTab coupons={coupons} onCreateCoupon={() => setShowCouponModal(true)} onDeleteCoupon={deleteCoupon} />}
        {activeTab === 'reviews' && <ReviewsTab reviews={reviews} accentColor={accentColor} />}
        {activeTab === 'history' && <HistoryTab filteredHistory={filteredHistory} hasMore={historyHasMore} loadingMore={historyLoadingMore} onLoadMore={() => fetchHistory(false)} getBranchName={getBranchName} onEditOrder={(order) => { setEditingOrder(order); setShowEditOrderModal(true); }} onDeleteOrder={handleDeleteSingleOrder} onDeleteAllHistory={handleDeleteAllHistory} />}
        {activeTab === 'billing' && <BillingTab config={config} accentColor={accentColor} onSubscribe={handleSubscribe} />}
        {activeTab === 'config' && <ConfigTab viewBranchId={viewBranchId} branches={branches} branchForm={branchForm} setBranchForm={setBranchForm} settingsForm={settingsForm} setSettingsForm={setSettingsForm} accentColor={accentColor} contrastTextColor={contrastTextColor} getBranchName={getBranchName} onOpenNewBranchModal={() => { setEditingBranch(null); setBranchForm({ name: '', address: '', phone: '', lat: '', lng: '' }); setShowBranchModal(true); }} onOpenEditBranchModal={(branch) => { setEditingBranch(branch); setBranchForm(branch); setShowBranchModal(true); }} onSetMainBranch={handleSetMainBranch} onDeleteBranch={handleDeleteBranch} onStoreImageUpload={handleStoreImageUpload} onSaveSettings={saveSettings} onGetBranchLocation={getBranchLocation} onUpdateBranchDetails={handleUpdateBranchDetails} />}
      </main>

      {/* MODALS */}
      {showBranchModal && <BranchModal branchForm={branchForm} setBranchForm={setBranchForm} editingBranch={editingBranch} onSave={handleSaveBranch} onClose={() => setShowBranchModal(false)} onGetLocation={getBranchLocation} />}
      {showTeamModal && <TeamModal newMember={newMember} setNewMember={setNewMember} branches={branches} onSubmit={handleInviteMember} onClose={() => setShowTeamModal(false)} />}
      {showRolesModal && <RolesModal onClose={() => setShowRolesModal(false)} />}
      {showEditOrderModal && editingOrder && <EditOrderModal editingOrder={editingOrder} setEditingOrder={setEditingOrder} onSave={saveOrderChanges} onClose={() => setShowEditOrderModal(false)} />}
      {showAssignRiderModal && selectedOrderForRider && <AssignRiderModal order={selectedOrderForRider} riders={riders} getBranchName={getBranchName} onAssign={handleAssignRider} onClose={() => setShowAssignRiderModal(false)} />}
      {showCreateModal && <CreateProductModal newProduct={newProduct} setNewProduct={setNewProduct} tempExtra={tempExtra} setTempExtra={setTempExtra} accentColor={accentColor} contrastTextColor={contrastTextColor} uploadingImage={uploadingImage} onImageUpload={handleImageUpload} onAddExtra={handleAddExtra} onRemoveExtra={handleRemoveExtra} onSubmit={handleCreateProduct} onClose={() => setShowCreateModal(false)} />}
      {showEditProductModal && editingProduct && <EditProductModal editingProduct={editingProduct} setEditingProduct={setEditingProduct} tempExtra={tempExtra} setTempExtra={setTempExtra} uploadingImage={uploadingImage} onImageUpload={(e) => handleImageUpload(e, true)} onAddExtra={handleAddExtra} onRemoveExtra={handleRemoveExtra} onSubmit={handleUpdateProduct} onClose={() => setShowEditProductModal(false)} />}
      {showPromoModal && promoTargetItem && <PromoModal promoTargetItem={promoTargetItem} promoConfig={promoConfig} setPromoConfig={setPromoConfig} onSubmit={handleCreatePromo} onClose={() => setShowPromoModal(false)} />}
      {showPriceModal && <PriceModal priceConfig={priceConfig} setPriceConfig={setPriceConfig} onApply={handleBulkPriceUpdate} onClose={() => setShowPriceModal(false)} />}
      {showCouponModal && <CouponModal newCoupon={newCoupon} setNewCoupon={setNewCoupon} accentColor={accentColor} contrastTextColor={contrastTextColor} onSubmit={handleCreateCoupon} onClose={() => setShowCouponModal(false)} />}
      {showCreateRiderModal && <CreateRiderModal newRider={newRider} setNewRider={setNewRider} branches={branches} accentColor={accentColor} contrastTextColor={contrastTextColor} onSubmit={handleCreateRider} onClose={() => setShowCreateRiderModal(false)} />}
    </div>
  );
}
