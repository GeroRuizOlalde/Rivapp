...
import {
  ...
  CreditCard, Phone, ChevronRight, MessageCircle, FileText, User, Globe, Shield, ChevronDown
} from 'lucide-react';
...
const MASTER_PIN = "1234"; // 🔒 PIN MAESTRO para acceder al modo "Dueño/Global"

// Definición de las pestañas del sistema
const ALL_TABS = [
  ...
  return (
    <div className="bg-[#1e1e1e] p-4 rounded-xl border border-white/5 shadow-xl group hover:border-white/10 transition-all relative overflow-hidden">
      ...
      {order.payment_method === 'mercadopago' && (
        <div className={`absolute top-0 right-0 px-3 py-1 text-[10px] font-bold rounded-bl-xl flex items-center gap-1 ${order.paid ? 'bg-green-500 text-white' : 'bg-[#009EE3] text-white'}`}>
          {order.paid ? <><Check size={10} /> PAGADO</> : 'MERCADO PAGO'}
        </div>
      )}
...
export default function AdminGastronomy() {
  const { store: config, refreshStore } = useStore();
  ...
  const [pendingBranchTarget, setPendingBranchTarget] = useState(null);
  ...
  // --- CONFIGURACIÓN DE TIENDA (SETTINGS) ---
  ...
  const [newRider, setNewRider] = useState({ name: '', phone: '', access_pin: '' });
  ...
  const fetchOrders = async () => {
    ...
    if (data) {
      setOrders(data);
      const visibleOrders = filterBranch === 'all' ? data : data.filter(o => o.branch_id === filterBranch);
  ...
  const fetchGlobalNotifications = async () => {
    const { data } = await supabase.from('global_notifications')
      .select('*')
      .eq('is_active', true)
  ...
  const fetchRiders = async () => { const { data } = await supabase.from('riders').select('*').eq('store_id', config.id).order('id'); if (data) setRiders(data || []); };
  ...
  useEffect(() => {
    ...
    fetchMenu(); fetchHistory(); fetchCoupons(); fetchRiders(); fetchReviews(); fetchGlobalNotifications(); fetchCRMData(); fetchBranches();
  ...
  const filteredOrders = useMemo(() => {
    if (filterBranch === 'all') return orders;
    return orders.filter(o => o.branch_id === filterBranch);
  }, [orders, filterBranch]);

  const filteredHistory = useMemo(() => {
    if (filterBranch === 'all') return historyOrders;
    return historyOrders.filter(o => o.branch_id === filterBranch);
  }, [historyOrders, filterBranch]);
  ...
  const handleSelectContext = (contextId) => {
    if (contextId === 'all') {
      setPendingBranchTarget('all');
      setShowAuthModal(true);
    } else {
      setFilterBranch(contextId);
      const branch = branches.find(b => b.id === contextId);
  ...
  useEffect(() => { calculateDashboardStats(); }, [orders, historyOrders, statsFilter, filterBranch]);
  ...
  const handleUpdateBranchDetails = async (e) => { e.preventDefault(); if (filterBranch === 'all') return; try { const { error } = await supabase.from('branches').update({ address: branchForm.address, phone: branchForm.phone, lat: branchForm.lat, lng: branchForm.lng }).eq('id', filterBranch); if (error) throw error; alert("¡Datos actualizados! 📍"); fetchBranches(); } catch (err) { alert("Error: " + err.message); } };
  ...
  const handleMarkAsPaid = async (id) => { if (!window.confirm("¿Confirmar que el cliente realizó el pago?")) return; await supabase.from('orders').update({ paid: true }).eq('id', id); fetchOrders(); };
  ...
  const saveSettings = async (e) => { e.preventDefault(); const { error } = await supabase.from('stores').update(settingsForm).eq('id', config.id); if (!error) { alert("¡Guardado correctamente! ✅"); if (refreshStore) refreshStore(); } else { alert("Error al guardar: " + error.message); } };
    ...
    <div className={`min-h-screen bg-[#0f0f0f] text-white flex flex-col lg:flex-row relative ${flash ? 'bg-orange-900/50' : ''} transition-colors duration-200`}>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-[200]">
          <div className="bg-[#1a1a1a] p-6 rounded-2xl w-full max-w-sm border border-white/10 shadow-2xl relative text-center">
            <button onClick={() => { setShowAuthModal(false); setPendingBranchTarget(null); }} className="absolute top-4 right-4 text-gray-400"><X size={20} /></button>
            <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4 text-[#d0ff00]"><Lock size={32} /></div>
            <h2 className="text-white font-bold text-xl mb-2">Acceso Restringido</h2>
            <p className="text-sm text-gray-400 mb-6">Ingresa tu PIN para acceder al Modo Dueño.</p>
            <form onSubmit={handleVerifyPin}>
              <input type="password" className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white text-center text-2xl font-bold tracking-widest mb-4 focus:border-[#d0ff00] transition-colors outline-none" placeholder="••••" maxLength={4} value={authPin} onChange={(e) => setAuthPin(e.target.value)} autoFocus />
              <button type="submit" className="w-full py-4 rounded-xl font-bold bg-[#d0ff00] text-black">Desbloquear</button>
            </form>
          </div>
        </div>
      )}
      ...
      <aside className="hidden lg:flex w-64 border-r border-white/5 flex-col p-4 bg-[#141414] h-screen sticky top-0 z-40 overflow-y-auto custom-scrollbar">
        ...
        {/* WIDGET DE SELECCIÓN RÁPIDA DE SUCURSAL */}
        <div className="mb-6 p-3 rounded-xl bg-white/5 border border-white/10 relative">
          <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest mb-1">Gestionando:</p>
          <div className="flex items-center gap-2">
            <Store size={16} className="text-[#d0ff00]" />
            <select
              value={filterBranch}
              onChange={(e) => handleSelectContext(e.target.value)}
              className="bg-transparent text-white font-bold text-sm w-full outline-none cursor-pointer appearance-none"
            >
              <option value="all" className="bg-[#141414] text-white">👑 Global (Dueño)</option>
              {branches.map(b => (
      ...
      <main className="flex-1 p-4 lg:p-8 overflow-y-auto h-screen bg-[#050505] pb-24 lg:pb-0">
        ...
        {/* HEADER MÓVIL CON SELECTOR RÁPIDO */}
        <div className="lg:hidden flex justify-between items-center mb-6">
          <div className="flex items-center gap-2 bg-[#1a1a1a] px-3 py-2 rounded-full border border-white/10 w-fit">
            ...
            >
              <option value="all" className="bg-[#1a1a1a]">Global</option>
              {branches.map(b => (
                <option key={b.id} value={b.id} className="bg-[#1a1a1a]">{b.name}</option>
              ))}
            </select>
            <ChevronDown size={12} className="text-gray-500 -ml-2" />
          </div>
        </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div><h1 className="text-3xl font-bold italic">Hola, {config?.name}</h1><p className="text-sm text-gray-400">Viendo datos de: <strong className="text-white">{filterBranch === 'all' ? 'Todas las Sucursales' : getBranchName(filterBranch)}</strong>.</p></div>
        ...
        {activeTab === 'riders' && (
          <div className="animate-in fade-in">
            <header className="flex justify-between items-center mb-8"><h1 className="text-3xl font-bold">Repartidores</h1><button onClick={() => setShowCreateRiderModal(true)} className="bg-blue-600 px-6 py-3 rounded-xl font-bold flex gap-2"><Plus /> Nuevo Rider</button></header>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{riders.map(r => (<div key={r.id} className="bg-[#1a1a1a] p-6 rounded-2xl border border-white/5 flex justify-between items-center"><div><h3 className="font-bold text-white flex items-center gap-2"><Bike size={18} /> {r.name}</h3><p className="text-xs text-gray-500 mt-1">PIN: <span className="font-mono bg-white/10 px-1 rounded text-white">{r.access_pin}</span></p></div><button onClick={() => deleteRider(r.id)} className="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500/20"><Trash2 size={18} /></button></div>))}</div>
          </div>
        )}
          ...
          <div className="space-y-6 animate-in fade-in max-w-2xl">
            {filterBranch === 'all' ? (
              ...
              <>
                ...
                {/* Configuración MercadoPago Global */}
                <div className="bg-blue-900/10 p-6 rounded-3xl border border-blue-500/20 space-y-4"><h3 className="text-blue-400 font-bold text-sm flex items-center gap-2"><CreditCard size={16} /> Integración Mercado Pago (Cobros Online)</h3><p className="text-xs text-gray-400">Pega aquí tus credenciales de producción para que tus clientes te paguen online.</p><div><label className="text-xs text-gray-500 uppercase font-bold">Access Token (Production)</label><input className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white mt-1" type="password" placeholder="APP_USR-..." value={settingsForm.mp_access_token} onChange={e => setSettingsForm({ ...settingsForm, mp_access_token: e.target.value })} /></div><div><label className="text-xs text-gray-500 uppercase font-bold">Public Key (Production)</label><input className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white mt-1" placeholder="TEST-..." value={settingsForm.mp_public_key} onChange={e => setSettingsForm({ ...settingsForm, mp_public_key: e.target.value })} /></div></div>
              ...
              <>
                <div className="flex items-center gap-4 mb-6">
                  <div>
                    <h1 className="text-3xl font-bold text-white flex items-center gap-2">{getBranchName(filterBranch)}</h1>
      ...
      {showEditOrderModal && editingOrder && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100]"><div className="bg-[#1a1a1a] p-6 rounded-2xl w-full max-w-md border border-white/10 shadow-2xl relative"><button onClick={() => setShowEditOrderModal(false)} className="absolute top-4 right-4 text-gray-400"><X size={20} /></button><h2 className="text-white font-bold text-xl mb-4">Editar Pedido #{editingOrder.id}</h2><form onSubmit={saveOrderChanges} className="space-y-4"><div><label className="text-xs text-gray-400">Nombre Cliente</label><input className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white" value={editingOrder.customer_name} onChange={e => setEditingOrder({ ...editingOrder, customer_name: e.target.value })} /></div><div className="grid grid-cols-2 gap-4"><div><label className="text-xs text-gray-400">Total ($)</label><input type="number" className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white" value={editingOrder.total} onChange={e => setEditingOrder({ ...editingOrder, total: e.target.value })} /></div><div><label className="text-xs text-gray-400">Estado</label><select className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white" value={editingOrder.status} onChange={e => setEditingOrder({ ...editingOrder, status: e.target.value })}><option value="pendiente">Pendiente</option><option value="confirmado">Confirmado</option><option value="listo">Listo</option><option value="entregado">Entregado</option><option value="archivado">Archivado</option><option value="rechazado">Rechazado</option></select></div></div><div><label className="text-xs text-gray-400">Pago</label><select className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white" value={editingOrder.payment_method} onChange={e => setEditingOrder({ ...editingOrder, payment_method: e.target.value })}><option value="efectivo">Efectivo</option><option value="mercadopago">MercadoPago</option></select></div><div><label className="text-xs text-gray-400">Nota</label><textarea className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white h-20 resize-none" value={editingOrder.note || ''} onChange={e => setEditingOrder({ ...editingOrder, note: e.target.value })} /></div><button type="submit" className="w-full py-3 rounded-xl font-bold bg-blue-600 text-white hover:bg-blue-500">Guardar Cambios</button></form></div></div>
      )}

      {showAssignRiderModal && selectedOrderForRider && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100]"><div className="bg-[#1a1a1a] p-6 rounded-2xl w-full max-w-sm border border-white/10 shadow-2xl relative"><button onClick={() => setShowAssignRiderModal(false)} className="absolute top-4 right-4 text-gray-400"><X size={20} /></button><h2 className="text-white font-bold text-xl mb-4">Asignar Rider</h2><div className="space-y-2">{riders.map(r => (<button key={r.id} onClick={() => handleAssignRider(r.id)} className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white flex justify-between items-center hover:border-white/30"><span className="font-bold flex items-center gap-2"><Bike size={16} /> {r.name}</span><span className="text-xs bg-white/10 px-2 py-1 rounded">Seleccionar</span></button>))}{riders.length === 0 && <p className="text-gray-500 text-center">No hay riders creados.</p>}</div></div></div>
      )}
      ...
      {showCreateRiderModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100]"><div className="bg-[#1a1a1a] p-6 rounded-2xl w-full max-w-sm border border-white/10 shadow-2xl relative"><button onClick={() => setShowCreateRiderModal(false)} className="absolute top-4 right-4 text-gray-400"><X size={20} /></button><h2 className="text-white font-bold text-xl mb-4">Nuevo Rider</h2><form onSubmit={handleCreateRider} className="space-y-3"><input className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white" placeholder="Nombre" value={newRider.name} onChange={e => setNewRider({ ...newRider, name: e.target.value })} /><input className="w-full bg-black/50 border border-white/10 p-3 rounded-xl text-white" placeholder="PIN de Acceso" value={newRider.access_pin} onChange={e => setNewRider({ ...newRider, access_pin: e.target.value })} /><button className="w-full py-3 rounded-xl font-bold" style={{ backgroundColor: accentColor, color: contrastTextColor }}>Crear Rider</button></form></div></div>
      )}
