...
export default function AdminGastronomy() {
  ...
  const handleMarkAsPaid = async (id) => { if (!window.confirm("¿Confirmar que el cliente realizó el pago?")) return; await supabase.from('orders').update({ paid: true, payment_status: 'paid' }).eq('id', id); fetchOrders(); };
  ...
  const saveOrderChanges = async (e) => { e.preventDefault(); await supabase.from('orders').update({ customer_name: editingOrder.customer_name, total: parseFloat(editingOrder.total), payment_method: editingOrder.payment_method, status: editingOrder.status, note: editingOrder.note }).eq('id', editingOrder.id); setShowEditOrderModal(false); fetchHistory(true); fetchOrders(); };
  const handleDeleteSingleOrder = async (id) => { if (window.confirm("¿Borrar?")) { await supabase.from('orders').delete().eq('id', id); fetchHistory(true); } };
