...
export default function AdminServices() {
  ...
  const handleUpdateStatus = async (id, newStatus, e) => { if(e) e.stopPropagation(); setAppointments(prev => prev.map(a => a.id === id ? {...a, status: newStatus} : a)); await supabase.from('appointments').update({ status: newStatus }).eq('id', id).eq('store_id', store.id); fetchAppointments(); };
  const handleDeleteAppointment = async (id, e) => { if(e) e.stopPropagation(); if(!window.confirm("¿Borrar turno permanentemente?")) return; await supabase.from('appointments').delete().eq('id', id).eq('store_id', store.id); fetchAppointments(); if(crmModal?.id === id) setCrmModal(null); };
  const sendReminder = (apt, e) => { if(e) e.stopPropagation(); const dateStr = new Date(apt.start_time).toLocaleDateString('es-AR', { weekday: 'long', day: 'numeric', hour: '2-digit', minute:'2-digit' }); const text = `Hola *${apt.customer_name}*! \u{1F44B}\nTe escribo de *${store.name}* para recordarte tu turno:\n\n\u2702\uFE0F ${apt.services?.name}\n\u{1F4C5} ${dateStr} hs\n\nPor favor confirmame si vas a poder asistir. \u00A1Gracias!`; window.open(`https://wa.me/${apt.customer_phone}?text=${encodeURIComponent(text)}`, '_blank'); };
  ...
  const handleSaveService = async (e) => { e.preventDefault(); if (editingService) await supabase.from('services').update({ name: newService.name, price: newService.price, duration_minutes: newService.duration }).eq('id', editingService.id).eq('store_id', store.id); else await supabase.from('services').insert([{ store_id: store.id, name: newService.name, price: newService.price, duration_minutes: newService.duration, active: true }]); setShowServiceModal(false); fetchServices(); };
  const deleteService = async (id) => { if(confirm("¿Borrar?")) { await supabase.from('services').delete().eq('id', id).eq('store_id', store.id); fetchServices(); } };
  ...
  const handleSaveStaff = async (e) => { e.preventDefault(); if (editingStaff) await supabase.from('staff').update({ name: newStaff.name, role: newStaff.role, avatar_url: newStaff.avatar_url }).eq('id', editingStaff.id).eq('store_id', store.id); else await supabase.from('staff').insert([{ store_id: store.id, ...newStaff }]); setShowStaffModal(false); fetchStaff(); };
  const deleteStaff = async (id) => { if(confirm("¿Borrar?")) { await supabase.from('staff').update({ active: false }).eq('id', id).eq('store_id', store.id); fetchStaff(); } };
  ...
  const deleteCoupon = async (id) => { if(confirm("¿Borrar?")) { await supabase.from('coupons').update({active: false}).eq('id', id).eq('store_id', store.id); fetchCoupons(); } };
  ...
  const saveInternalNote = async (id, note) => { await supabase.from('appointments').update({ internal_notes: note }).eq('id', id).eq('store_id', store.id); fetchAppointments(); };
