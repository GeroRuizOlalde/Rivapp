...
import { 
  ...
  User, Users, Phone, RefreshCw, Loader2, TrendingUp, DollarSign, UserPlus, 
  Store, Save, MapPin, Image as ImageIcon, Upload, Camera, UserCog, History, FileText,
  ToggleLeft, ToggleRight, Tag, Bell, CreditCard, Crown, Check, XCircle, AlertTriangle, X, Lock, Edit, Volume2, VolumeX, LogOut,
  Inbox
} from 'lucide-react';
...
// --- SONIDO ---
const BELL_SOUND = "/sounds/ding.mp3"; 
...
const ALL_TABS = [
  ...
  { id: 'team', label: 'Equipo', icon: UserCog, proOnly: true }, // Marcamos las PRO
  { id: 'servicios', label: 'Servicios', icon: Briefcase },
  { id: 'marketing', label: 'Marketing', icon: Tag, proOnly: true }, // Marcamos las PRO
...
export default function AdminServices() {
  ...
  const { features, isExpired, canAccessAdmin } = useEntitlements(store); // 🟢 Usamos el hook
  ...
  const toggleSound = () => {
      ...
      if (newState) {
          ...
          }).catch(e => console.log("Permiso de audio pendiente"));
      }
  };
  ...
  // 🟢 FILTRO DE TABS USANDO ENTITLEMENTS
  const visibleTabs = useMemo(() => {
    if (!store) return [];
    if (features.hasProTabs) return ALL_TABS; // Si tiene permisos PRO (o es Trial/Demo) ve todo
    return ALL_TABS.filter(tab => !tab.proOnly); // Si no, filtramos
  }, [features]);
  ...
  const fetchGlobalNotifications = async () => {
    ...
    if (data && data.length > 0) {
        ...
        if (filtered.length > 0) {
            ...
            if (new Date(newest.created_at) > fiveMinAgo) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime_services')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'appointments', filter: `store_id=eq.${store.id}` }, () => { fetchAppointments(); })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'global_notifications' }, () => { fetchGlobalNotifications(); })
      .subscribe();
  ...
  const fetchAppointments = async () => { 
      setLoadingApts(true); 
      const { data } = await supabase.from('appointments').select('*, services(name, duration_minutes, price), staff(name)').eq('store_id', store.id).order('start_time', { ascending: true }); 
      if (data) {
          setAppointments(data);
          const currentPending = data.filter(a => a.status === 'pendiente').length;
          if (prevAppointmentsCount.current !== null && currentPending > prevAppointmentsCount.current) {
             playNotification();
          }
          prevAppointmentsCount.current = currentPending;
      }
  ...
  const uploadImage = async (file) => { try { const ext = file.name.split('.').pop(); const name = `${store.id}/${Date.now()}.${ext}`; const { error } = await supabase.storage.from('store-assets').upload(name, file); if(error) throw error; const { data } = supabase.storage.from('store-assets').getPublicUrl(name); return data.publicUrl; } catch (e) { alert(e.message); return null; } };
  const handleFileChange = async (e, f, setter = setProfileForm) => { const file = e.target.files[0]; if(!file) return; setUploading(true); const url = await uploadImage(file); if(url) { if(f==='avatar_url') setNewStaff(p=>({...p,avatar_url:url})); else setter(p=>({...p,[f]:url})); } setUploading(false); };
  
  const sendReminder = (apt, e) => { 
      if(e) e.stopPropagation(); 
      const dateStr = new Date(apt.start_time).toLocaleDateString('es-AR', { weekday: 'long', day: 'numeric', hour: '2-digit', minute:'2-digit' }); 
      const text = `Hola *${apt.customer_name}*! 👋%0ATe escribo de *${store.name}* para recordarte tu turno:%0A%0A✂️ ${apt.services?.name}%0A📅 ${dateStr} hs%0A%0APor favor confirmame si vas a poder asistir. ¡Gracias!`; 
      window.open(`https://wa.me/${apt.customer_phone}?text=${text}`, '_blank'); 
  };
  ...
  const handleUpdateStatus = async (id, newStatus, e) => {
      ...
      if (!error) {
          fetchAppointments();
          alert(newStatus === 'confirmado' ? "Turno aceptado ✅" : "Turno rechazado ❌");
      }
  };

  const handleSubscribe = async () => {
      if (!store?.id) return alert("Error: No se identificó la tienda.");
      const btn = document.activeElement;
      if(btn) btn.innerText = "Procesando...";
      try {
          const { data, error } = await supabase.functions.invoke('create-checkout', { body: JSON.stringify({ store_id: store.id, price: 40000, title: `Suscripción Pro - ${store.name}`, domain_url: window.location.origin }), headers: { "Content-Type": "application/json" } });
          if (error) throw new Error(error.message);
          if (data?.init_point) window.open(data.init_point, '_blank');
      } catch (err) { alert("Error de conexión."); } finally { if(btn) btn.innerText = "Pasarme a PRO"; }
  };

  const openServiceModal = (service = null) => {
      if (service) {
          setNewService({ name: service.name, price: service.price, duration: service.duration_minutes });
  ...
  const handleSaveService = async (e) => { 
      e.preventDefault(); 
      if (editingService) {
          await supabase.from('services').update({ name: newService.name, price: newService.price, duration_minutes: newService.duration }).eq('id', editingService.id);
          alert("Servicio actualizado ✅");
      } else {
          await supabase.from('services').insert([{ store_id: store.id, name: newService.name, price: newService.price, duration_minutes: newService.duration, active: true }]);
  ...
  const openStaffModal = (staff = null) => {
      if (staff) {
          setNewStaff({ name: staff.name, role: staff.role, avatar_url: staff.avatar_url });
  ...
  const handleSaveStaff = async (e) => { 
      e.preventDefault(); 
      if (editingStaff) {
          await supabase.from('staff').update({ name: newStaff.name, role: newStaff.role, avatar_url: newStaff.avatar_url }).eq('id', editingStaff.id);
          alert("Miembro actualizado ✅");
      } else {
          await supabase.from('staff').insert([{ store_id: store.id, ...newStaff }]);
  ...
  const deleteStaff = async (id) => { if(!window.confirm("¿Seguro?")) return; await supabase.from('staff').update({ active: false }).eq('id', id); fetchStaff(); };
  const handleCreateCoupon = async (e) => { e.preventDefault(); await supabase.from('coupons').insert([{ store_id: store.id, code: newCoupon.code.toUpperCase(), discount: newCoupon.discount, active: true }]); setShowCouponModal(false); setNewCoupon({code:'', discount:10}); fetchCoupons(); alert("Cupón creado ✅"); };
  ...
  const handleSaveSchedule = async () => { 
      const upserts = schedules.map(s => ({ store_id: store.id, day_of_week: s.day_of_week, open_time: s.open_time, close_time: s.close_time, is_closed: s.is_closed })); 
      await supabase.from('store_schedules').upsert(upserts, { onConflict: 'store_id, day_of_week' }); 
      await supabase.from('stores').update({ enable_multislots: slotsConfig.enable_multislots, max_concurrent_slots: slotsConfig.max_concurrent_slots }).eq('id', store.id);
      alert("Configuración guardada ✅");
  };
  
  const handleSaveProfile = async (e) => { e.preventDefault(); await supabase.from('stores').update({ name: profileForm.name, phone: profileForm.phone, address: profileForm.address, logo_url: profileForm.logo_url, banner_url: profileForm.banner_url, color_accent: profileForm.color_accent, enable_staff_selection: profileForm.enable_staff_selection, enable_payments: profileForm.enable_payments, mp_public_key: profileForm.mp_public_key, mp_access_token: profileForm.mp_access_token }).eq('id', store.id); alert("Perfil guardado"); };
  ...
  // 🟢 BLOQUEO POR VENCIMIENTO CENTRALIZADO
  if (!canAccessAdmin) {
    return (
        <div className="min-h-screen bg-black flex items-center justify-center p-6 text-center animate-in fade-in">
            <div className="max-w-md bg-[#111] p-10 rounded-[3rem] border border-red-500/30 shadow-2xl">
                ...
                <p className="text-gray-400 mb-8">Tu periodo de servicio ha finalizado. Para seguir gestionando tu local, activa tu plan.</p>
                <button onClick={handleSubscribe} className="w-full py-4 bg-white text-black font-black rounded-2xl hover:bg-[#d0ff00] transition-all flex items-center justify-center gap-2">Pagar Ahora <ChevronRight size={20}/></button>
            </div>
        </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans flex flex-col md:flex-row relative">
      <AnimatePresence>
        {activeAlert && (
            <motion.div 
            ...
            >
                <div className="bg-[#0a0a0a] border-2 border-blue-600 p-5 rounded-2xl shadow-2xl flex items-start gap-4">
                    <div className="bg-blue-600/10 p-3 rounded-xl text-blue-500"><Bell size={24}/></div>
                    <div className="flex-1">
                        <h4 className="font-black text-blue-500 text-lg leading-tight mb-1">{activeAlert.title}</h4>
                        <p className="text-sm text-gray-300">{activeAlert.message}</p>
                    </div>
                    <button onClick={() => dismissMessage(activeAlert.id)} className="text-gray-500 hover:text-white transition-colors"><X size={20}/></button>
                </div>
            </motion.div>
        )}
      </AnimatePresence>
      ...
      <aside className="w-full md:w-64 bg-[#0a0a0a] border-r border-white/5 p-6 flex flex-col gap-6 sticky top-0 h-screen overflow-y-auto">
        <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white overflow-hidden border border-white/10 shrink-0" style={{backgroundColor: accentColor}}>
                {profileForm.logo_url ? <img src={profileForm.logo_url} className="w-full h-full object-cover"/> : store?.name.substring(0,2).toUpperCase()}
            </div>
            <div className="min-w-0"><h2 className="font-bold leading-none text-white truncate">{profileForm.name || store?.name}</h2><span className="text-xs" style={{color: accentColor}}>Panel Admin</span></div>
        </div>
        <nav className="flex flex-col gap-2 flex-1">
            {visibleTabs.map(tab => (
              <button key={tab.id} onClick={()=>setActiveTab(tab.id)} className={`p-3 rounded-xl flex gap-3 transition-all items-center ${activeTab===tab.id ? 'text-white shadow-lg' : 'text-gray-400 hover:bg-white/5'}`} style={activeTab === tab.id ? {backgroundColor: accentColor + '33', color: accentColor} : {}}>
                  ...
                  <span>{tab.label}</span>
                  {tab.id === 'inbox' && pendingAppointments.length > 0 && (
                      <span className="ml-auto bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold">{pendingAppointments.length}</span>
                  )}
              </button>
            ))}
        </nav>
        <div className="mt-auto pt-4 border-t border-white/10 flex flex-col gap-2">
            <button onClick={toggleSound} className={`w-full p-2 rounded-xl font-bold text-sm flex items-center justify-center gap-2 ${isSoundEnabled ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'}`}>
                {isSoundEnabled ? <Volume2 size={16}/> : <VolumeX size={16}/>} {isSoundEnabled ? "Sonido ON" : "Sonido OFF"}
            </button>
      ...
      <main className="flex-1 p-6 md:p-8 overflow-y-auto h-screen bg-[#050505]">
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div><h1 className="text-3xl font-bold italic">Hola, {store?.name}</h1><p className="text-sm text-gray-500">Resumen de ventas.</p></div>
              <div className="flex gap-2 w-full md:w-auto">
                <a href={`/${store?.slug}`} target="_blank" rel="noopener noreferrer" className="bg-[#1a1a1a] border border-white/10 text-white p-3 rounded-xl hover:bg-white/10 transition-all flex items-center gap-2 flex-1 md:flex-none justify-center group"><ExternalLink size={20} style={{color: accentColor}} className="group-hover:scale-110 transition-transform"/><span className="font-bold text-sm">Ver Local</span></a>
                <button onClick={() => refreshAllData()} className="p-3 bg-[#1a1a1a] border border-white/10 rounded-xl text-gray-400 hover:text-white"><RefreshCw size={20}/></button>
              </div>
            </div>

            {globalNotifications.length > 0 && (
                <div className="bg-[#111] p-6 rounded-3xl border border-blue-500/20 relative overflow-hidden shadow-lg">
                    <div className="absolute top-0 right-0 bg-blue-600/5 w-32 h-32 blur-3xl rounded-full -mr-10 -mt-10"></div>
                    <h3 className="font-black text-blue-500 text-sm uppercase tracking-widest mb-4 flex items-center gap-2"><Bell size={16}/> Comunicados de Rivapp</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-[#111] p-6 rounded-3xl border border-white/5">
                  <div className="flex justify-between items-center mb-4"><div className="p-3 rounded-2xl" style={{backgroundColor: accentColor + '22', color: accentColor}}><TrendingUp/></div><span className="text-xs font-bold text-gray-500 uppercase">Hoy</span></div>
                  <h3 className="text-gray-400 text-sm font-medium">Ingresos estimados</h3>
                  <div className="text-3xl font-bold">${stats.incomeToday}</div>
                </div>
                <div className="bg-[#111] p-6 rounded-3xl border border-white/5">
                  <div className="flex justify-between items-center mb-4"><div className="p-3 bg-purple-600/10 rounded-2xl text-purple-500"><CalendarIcon/></div><span className="text-xs font-bold text-gray-500 uppercase">Hoy</span></div>
                  <h3 className="text-gray-400 text-sm font-medium">Turnos agendados</h3>
                  <div className="text-3xl font-bold">{stats.todayCount}</div>
                </div>
                <div className="bg-[#111] p-6 rounded-3xl border border-white/5">
                  <div className="flex justify-between items-center mb-4"><div className="p-3 bg-green-600/10 rounded-2xl text-green-500"><Clock/></div><span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Próximo</span></div>
                  {stats.nextApt ? (<div><div className="text-xl font-bold truncate">{stats.nextApt.customer_name}</div><div className="text-sm text-gray-500">{new Date(stats.nextApt.start_time).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})} • {stats.nextApt.services?.name}</div></div>) : <div className="text-gray-600 italic">Sin más turnos.</div>}
                </div>
            </div>
            ...
            <div className="bg-[#111] p-8 rounded-3xl border border-white/5">
                ...
                <div style={{ width: '100%', height: 350 }}> 
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats.chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#222"/><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill:'#666', fontSize:12}}/><YAxis axisLine={false} tickLine={false} tick={{fill:'#666', fontSize:12}}/><Tooltip cursor={{fill: '#222'}} contentStyle={{backgroundColor:'#111', border:'1px solid #333', borderRadius:'12px'}}/><Bar dataKey="ingresos" fill={accentColor} radius={[6, 6, 0, 0]} barSize={40}/></BarChart>
                  </ResponsiveContainer>
                </div>
            </div>
          </div>
        ...
        {activeTab === 'inbox' && (
            <div className="space-y-6 animate-in fade-in">
                <header className="flex justify-between items-center mb-4">
                    <div>
                        <h1 className="text-3xl font-bold">Solicitudes de Turnos</h1>
                        <p className="text-gray-400">Turnos web pendientes de confirmación.</p>
                    </div>
                    {pendingAppointments.length > 0 && (
                        <div className="bg-red-500/10 text-red-500 px-4 py-2 rounded-xl font-bold border border-red-500/20 animate-pulse">
                ...
                {pendingAppointments.length === 0 ? (
                    <div className="bg-[#111] p-12 rounded-3xl border border-white/5 text-center flex flex-col items-center">
                        <div className="bg-white/5 p-6 rounded-full mb-4"><Inbox size={48} className="text-gray-500"/></div>
                        <h3 className="text-xl font-bold text-white">¡Todo al día!</h3>
                        <p className="text-gray-500 mt-2">No tienes nuevas solicitudes de turnos.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {pendingAppointments.map(apt => (
                            <div key={apt.id} className="bg-[#1a1a1a] p-5 rounded-2xl border border-yellow-500/30 relative overflow-hidden shadow-lg hover:shadow-yellow-900/20 transition-all">
                                ...
                                <div className="mb-4">
                                    <h3 className="font-bold text-xl text-white">{apt.customer_name}</h3>
                                    <p className="text-sm text-gray-400">{apt.customer_phone}</p>
                                </div>
                                <div className="space-y-2 mb-6">
                                    <div className="flex items-center gap-2 text-sm">
                                        <Briefcase size={16} className="text-gray-500"/>
                                        <span className="font-bold">{apt.services?.name}</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-sm">
                                        <CalendarIcon size={16} className="text-gray-500"/>
                                        <span className="font-mono bg-white/10 px-2 rounded">{new Date(apt.start_time).toLocaleDateString()}</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-sm">
                                        <Clock size={16} className="text-gray-500"/>
                                        <span className="font-mono bg-white/10 px-2 rounded">{new Date(apt.start_time).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
                                    </div>
                                    {apt.staff && (
                                        <div className="flex items-center gap-2 text-sm text-blue-400">
                                            <UserCog size={16}/>
                                            <span>Con {apt.staff.name}</span>
                                        </div>
                                    )}
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    <button 
                                        ...
                                        className="bg-red-500/10 text-red-500 py-3 rounded-xl font-bold text-sm hover:bg-red-500 hover:text-white transition-colors"
                                    >
                                        Rechazar
                                    </button>
                                    <button 
                                        onClick={(e) => handleUpdateStatus(apt.id, 'confirmado', e)}
                                        className="bg-green-500 text-black py-3 rounded-xl font-bold text-sm hover:bg-green-400 transition-colors flex items-center justify-center gap-2"
                                    >
        ...
        {activeTab === 'agenda' && ( <div className="flex flex-col md:flex-row gap-8 h-full animate-in fade-in"><div className="w-full md:w-[400px] shrink-0 bg-[#111] p-6 rounded-3xl border border-white/5 h-fit"><header className="flex justify-between items-center mb-6"><button onClick={() => changeMonth(-1)}><ChevronLeft/></button><h2 className="font-bold capitalize">{MONTHS[currentDate.getMonth()]} {currentDate.getFullYear()}</h2><button onClick={() => changeMonth(1)}><ChevronRight/></button></header><div className="grid grid-cols-7 gap-2 text-center text-sm">{getDaysInMonth(currentDate).map((d, i) => d ? <button key={i} onClick={() => setSelectedDate(d)} className={`aspect-square rounded-xl flex flex-col items-center justify-center transition-colors ${isSameDay(d, selectedDate) ? 'text-white' : 'hover:bg-white/5'}`} style={isSameDay(d, selectedDate) ? {backgroundColor: accentColor} : {}}>{d.getDate()}{getDailyLoad(d) > 0 && <div className="w-1 h-1 bg-white rounded-full mt-1"></div>}</button> : <div key={i}></div>)}</div></div><div className="flex-1"><header className="mb-6 flex justify-between items-end"><h1 className="text-3xl font-bold">{selectedDate.toLocaleDateString()}</h1><button onClick={() => setShowManualModal(true)} className="px-4 py-2 rounded-lg font-bold flex items-center gap-2 text-white" style={{backgroundColor: accentColor}}><Plus size={16}/> Agendar</button></header><div className="space-y-3">{filteredAppointments.map(apt => ( <div key={apt.id} onClick={() => setCrmModal(apt)} className="bg-[#111] p-5 rounded-2xl border border-white/5 hover:border-blue-500/30 cursor-pointer transition-all group flex justify-between items-center relative overflow-hidden"><div className="absolute left-0 top-0 bottom-0 w-1" style={{backgroundColor: apt.status==='confirmado' ? '#22c55e' : '#eab308'}}></div><div className="flex gap-4 items-center pl-3"><div className="text-center"><div className="text-lg font-bold">{new Date(apt.start_time).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div></div><div><h3 className="font-bold text-lg">{apt.customer_name}</h3><p className="text-sm text-gray-400 flex items-center gap-2">{apt.services?.name} {apt.staff && <span className="bg-white/10 px-2 py-0.5 rounded text-[10px] flex items-center gap-1"><UserCog size={10}/> {apt.staff.name}</span>}</p></div></div><div className="flex items-center gap-4"><div className="text-right"><div className="text-green-500 font-bold">${apt.price_paid || apt.services?.price}</div>{apt.coupon_code && <div className="text-[10px] text-blue-400 bg-blue-500/10 px-1 rounded flex items-center justify-end gap-1"><Tag size={10}/> {apt.coupon_code}</div>}{apt.payment_method === 'mercadopago' && <div className="text-[8px] text-white bg-blue-600 px-1 rounded mt-1 font-bold">MERCADO PAGO</div>}</div><button onClick={(e) => handleDeleteAppointment(apt.id, e)} className="p-3 bg-red-500/10 hover:bg-red-500 hover:text-white rounded-xl text-red-500 transition-colors" title="Borrar Turno"><Trash2 size={18}/></button><button onClick={(e) => sendReminder(apt, e)} className="p-3 bg-white/5 hover:bg-green-600 hover:text-white rounded-xl text-gray-400 transition-colors" title="Enviar recordatorio por WhatsApp"><Bell size={18}/></button></div></div> ))}</div></div></div> )}
        
        {activeTab === 'team' && ( 
            <div className="animate-in fade-in"><header className="flex justify-between items-center mb-8"><h1 className="text-3xl font-bold">Nuestro Equipo</h1><button onClick={() => openStaffModal()} className="px-6 py-3 rounded-xl font-bold flex items-center gap-2 text-white" style={{backgroundColor: accentColor}}><Plus size={20}/> Agregar</button></header><div className="grid grid-cols-1 md:grid-cols-4 gap-6">{staffList.map(member => (<div key={member.id} className="bg-[#111] p-6 rounded-2xl border border-white/5 flex flex-col items-center text-center group relative hover:border-white/20 transition-all"><div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => openStaffModal(member)} className="p-2 bg-white/10 rounded-lg hover:bg-white hover:text-black"><Edit size={16}/></button><button onClick={() => deleteStaff(member.id)} className="p-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-white"><Trash2 size={16}/></button></div><div className="w-20 h-20 rounded-full bg-gray-800 mb-4 overflow-hidden border-2 border-white/10 group-hover:border-blue-500 transition-colors">{member.avatar_url ? <img src={member.avatar_url} className="w-full h-full object-cover"/> : <UserCog className="w-full h-full p-4 text-gray-500"/>}</div><h3 className="font-bold text-lg">{member.name}</h3><p className="text-sm" style={{color: accentColor}}>{member.role}</p></div>))}</div></div> )}
